...
import { Check, CheckCircle2, ChevronLeft, ChevronRight, File, Minus, Package, Plus, Send, Trash2, UploadCloud, X } from 'lucide-react';
...
const describeItem = (item: RfqCartItem) => {
  if (item.specSummary) return item.specSummary;
  const details = [
    item.material,
    item.selectedColor ? `Color: ${item.selectedColor}` : '',
    item.selectedFinish ? `Finish: ${item.selectedFinish}` : '',
    item.selectedThickness ? `Thickness: ${item.selectedThickness}` : '',
    item.selectedEdge ? `Edge: ${item.selectedEdge}` : '',
  ].filter(Boolean);
  return details.join(' · ') || 'Specifications to be confirmed';
};
...
export const RfqModal: React.FC<RfqModalProps> = ({
  ...
  const itemSummary = cartItems.map((item) => `${item.quantity} x ${item.title}${item.sku ? ` (${item.sku})` : ''} - ${describeItem(item)}`).join('\n');
  ...
  return (
    <Modal onClose={close} ariaLabelledBy="rfq-title" panelClassName="wr-rfq-dialog" closeOnBackdrop={false}>
      <header className="wr-modal-header">
        <div><span className="wr-eyebrow">B2B inquiry builder</span><h2 id="rfq-title">RFQ Builder</h2></div>
      ...
      <div className="wr-rfq-body">
        {step === 'items' && (
          <section className="wr-rfq-items">
            {(siteConfig.web3FormsAccessKey || cartItems.length > 0) && <div className="wr-rfq-section-title"><div><h3>{t(currentLocale, 'review')}</h3><p>Review the selected products, specifications, and quantities before adding buyer details.</p></div>{cartItems.length > 0 && <button className="wr-button wr-button--ghost" onClick={onClearCart}><Trash2 />{t(currentLocale, 'clear')}</button>}</div>}
            {!cartItems.length ? (siteConfig.web3FormsAccessKey ? <div className="wr-empty-state"><Package /><h3>Your RFQ is empty.</h3><p>Add products from the catalog, then return here to send the project inquiry.</p></div> : null) : cartItems.map((item) => (
              <article key={item.id} className="wr-rfq-item">
                <div><small>{item.type}{item.sku ? ` · ${item.sku}` : ''}</small><h4>{item.title}</h4><p>{describeItem(item)}</p></div>
            ...
            {(siteConfig.web3FormsAccessKey || cartItems.length > 0) && <footer><span>{cartItems.reduce((sum, item) => sum + item.quantity, 0)} total units</span>{siteConfig.web3FormsAccessKey && <button className="wr-button wr-button--primary" disabled={!cartItems.length} onClick={() => setStep('details')}>{t(currentLocale, 'continue')}<ChevronRight /></button>}</footer>}
        ...
        {step === 'details' && (
          <form className="wr-rfq-form" onSubmit={(event) => { event.preventDefault(); setStep('review'); }}>
            <div className="wr-rfq-section-title"><div><h3>Buyer and project details</h3><p>Fields marked with * are required for a useful quotation response.</p></div></div>
            <div className="wr-form-grid">
              ...
              <label className="wr-form-grid__wide"><span>Notes, drawing references, edge details, or required documents</span><textarea rows={4} value={formData.customNotes} onChange={(event) => setFormData({ ...formData, customNotes: event.target.value })} /></label>
            </div>
