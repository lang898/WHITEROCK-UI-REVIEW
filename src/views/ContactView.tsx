...
export const ContactView: React.FC<ContactViewProps> = ({ currentLocale, onOpenShareModal }) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      subject: `WHITEROCK ${formData.inquiryType || 'website'} inquiry from ${formData.company || formData.name}`,
    ...
    <div className="wr-contact-page">
      <header className="wr-contact-hero wr-section-band">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <button className="wr-button wr-button--primary" type="button" onClick={openRfqBuilder}><FileText aria-hidden="true" />Open RFQ Builder</button>
          <a className="wr-button wr-button--secondary" href={`mailto:${siteConfig.email}`}><Mail aria-hidden="true" />Email WHITEROCK</a>
        </div>
      </header>
      ...
      <section className="wr-contact-main wr-section-band wr-section-band--mist" aria-labelledby="contact-form-title">
        <aside className="wr-contact-details">
          ...
          {onOpenShareModal && (
            <button className="wr-button wr-button--secondary" onClick={() => onOpenShareModal({ title: 'WHITEROCK contact', text: 'Contact WHITEROCK for stone samples, technical support, documentation, and project communication.', type: 'site' })}>
        ...
        <div className="wr-contact-form-panel">
          {!siteConfig.web3FormsAccessKey ? (
            ...
            <div className="wr-contact-success" role="status">
              ...
              <span className="wr-eyebrow">Message received</span>
              <h2>Thank you, {formData.name}.</h2>
              <p className="wr-submission-notice wr-submission-notice--success">{submissionNote}</p>
              <button className="wr-button wr-button--secondary" onClick={() => setIsSubmitted(false)}>Send another message</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} aria-labelledby="contact-form-title" aria-busy={isSubmitting}>
              <div className="wr-contact-form-heading">
                <span className="wr-eyebrow">General inquiry</span>
                <h2 id="contact-form-title">How can we help?</h2>
                <p>For pricing, quantities, destination, and production drawings, use the RFQ Builder instead.</p>
              </div>
              <div className="wr-form-grid">
                ...
                <label className="wr-contact-field" htmlFor="contact-inquiry-type">
                  <span>Inquiry type *</span>
                  <select id="contact-inquiry-type" required value={formData.inquiryType} onChange={(event) => setFormData({ ...formData, inquiryType: event.target.value })}>
                    <option value="">Select an inquiry type</option>
                    <option>General question</option>
                    <option>Samples and material review</option>
                    <option>Technical documents</option>
                    <option>Supplier qualification</option>
                    <option>Existing order or production support</option>
                    <option>Other</option>
                  </select>
                </label>
                <label className="wr-contact-field wr-form-grid__wide" htmlFor="contact-message">
                  <span>Message *</span>
                  <textarea id="contact-message" required rows={6} value={formData.message} onChange={(event) => setFormData({ ...formData, message: event.target.value })} placeholder="Tell us what you need and include any relevant order, product, material, or document reference." />
                </label>
              </div>
