...
export const ContactView: React.FC<ContactViewProps> = ({ currentLocale, onOpenShareModal }) => {
  ...
  const copyEmail = async () => {
    try { await navigator.clipboard.writeText(siteConfig.email); setCopied(true); window.setTimeout(() => setCopied(false), 1800); }
    catch { window.location.href = `mailto:${siteConfig.email}`; }
  };

  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (isSubmitting || !siteConfig.web3FormsAccessKey) return;
    setSubmissionError(''); setIsSubmitting(true);
    ...
    try { await submitInquiry({ accessKey: siteConfig.web3FormsAccessKey, fields: payload, files: [] }); setSubmissionNote('Thank you. We will reply by email shortly.'); setIsSubmitted(true); }
    catch { setSubmissionError(`Something went wrong or the request timed out. Please email ${siteConfig.email} directly.`); }
  ...
  return (
    <div className="wr-contact-page">
      <LandingHero
        eyebrow="Contact"
        title="One direct line to the Vietnam manufacturing team."
        description="Use this page for technical questions, supplier qualification, samples, documents, or existing-order support. New project pricing belongs in the RFQ Builder so the drawing and specifications stay with the inquiry."
        image="/assets/owner/enhanced/material-staging-hall-enhanced.jpg"
        imageWebp="/assets/owner/enhanced/material-staging-hall-enhanced-1280.webp"
        imageAlt="Finished stone components staged inside the Vietnam factory"
        actions={<><button className="wr-button wr-button--primary" type="button" onClick={openRfqBuilder}><FileText />Open RFQ Builder</button><a className="wr-button wr-button--secondary" href={routePath('samples')}><Package />Request samples</a></>}
      />
