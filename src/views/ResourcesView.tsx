...
export const ResourcesView: React.FC<ResourcesViewProps> = ({ currentLocale, setCurrentTab = () => undefined, section }) => {
  ...
  const filtered = documents.filter((item) =>
    (!sectionType || item.documentType === sectionType)
  ...
  return (
    <div className="wr-resources-page">
      <header className="wr-landing-hero wr-landing-hero--resources">
        <span className="wr-eyebrow">Resources</span>
        <h1>{heading?.title || 'Technical resources for specification and production.'}</h1>
        <p>{heading?.copy || 'Find documents, CAD support, care guidance, packing references, sample support, and answers for stone programs.'}</p>
      </header>
      ...
      {showLibrary && <section className="wr-document-center" aria-labelledby="document-center-title">
