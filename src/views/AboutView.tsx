  ...
  return (
    <div className="wr-about-page">
      <header className="wr-about-hero">
        ...
        <div />
        <section><span className="wr-eyebrow wr-eyebrow--light">{siteConfig.displayBrand}</span><h1>Stone experience carried into direct Vietnam manufacturing.</h1><p>Natural and engineered stone products are fabricated in Dong Nai Province for international B2B buyers, with specifications and commercial terms confirmed for each project.</p></section>
      </header>

      <section className="wr-about-facts wr-section-band" aria-label="Company figures">{factory.stats.slice(0, 4).map((stat) => <article key={stat.label}><strong>{stat.value}</strong><span>{stat.label}</span></article>)}</section>

      <section className="wr-about-story wr-section-band wr-section-band--mist" aria-labelledby="about-story-title">
        <div><span className="wr-eyebrow">Our manufacturing approach</span><h2 id="about-story-title">Craft knowledge, industrial discipline, clear order documents.</h2><p>Existing public company materials describe more than two decades of stone-industry experience and the current Vietnam facility profile. Published plant area and output figures remain subject to owner confirmation before they are treated as permanent public claims.</p><Button variant="secondary" onClick={() => setCurrentTab('factory')}>Review the factory<ArrowRight /></Button></div>
      ...
      <section className="wr-about-capabilities wr-section-band" aria-labelledby="about-capabilities-title"><div className="wr-section-heading wr-section-intro"><span className="wr-eyebrow">Core programs</span><h2 id="about-capabilities-title">One manufacturing partner across the stone package.</h2><p>The scope begins with a sample, drawing, quantity, destination, and schedule, then moves through written approval and order-specific production controls.</p></div><div>{capabilities.map((capability) => <article key={capability}><Check /><span>{capability}</span></article>)}</div></section>
