...
export const AboutView: React.FC<AboutViewProps> = ({ setCurrentTab, section }) => {
  ...
  const title = section === 'story' ? 'A stone program starts with the material and the drawing.' : section === 'vietnam' ? 'Direct manufacturing in Dong Nai, Vietnam.' : 'Stone experience carried into direct Vietnam manufacturing.';
  const heroImage = section === 'story' ? '/assets/owner/enhanced/vanity-workshop-overhead-enhanced.jpg' : '/assets/owner/enhanced/factory-exterior-enhanced.jpg';
  const heroWebp = section === 'story' ? '/assets/owner/enhanced/vanity-workshop-overhead-enhanced-1280.webp' : '/assets/owner/enhanced/factory-exterior-enhanced-1280.webp';
  const heroAvif = section === 'story' ? '/assets/owner/enhanced/vanity-workshop-overhead-enhanced-1280.avif' : '/assets/owner/enhanced/factory-exterior-enhanced-1280.avif';
  const heroAlt = section === 'story' ? 'Vanity tops arranged across production worktables' : 'Stone manufacturing facility in Dong Nai, Vietnam';

  return (
    <div className="wr-about-page">
      <LandingHero eyebrow={section ? `About · ${section === 'story' ? 'Our Story' : 'Vietnam Manufacturing'}` : siteConfig.displayBrand} title={title} description="Natural and engineered stone products are fabricated for B2B programs with material selection, approved drawings, production controls, inspection, packing, and shipment preparation connected in one workflow." image={heroImage} imageWebp={heroWebp} imageAvif={heroAvif} imageAlt={heroAlt} />
      ...
      {showStory && <section className="wr-about-story wr-section-band wr-section-band--mist" aria-labelledby="about-story-title"><div><span className="wr-eyebrow">Our story</span><h2 id="about-story-title">Material knowledge becomes useful when the drawing controls the work.</h2><p>WHITEROCK COMPANY LIMITED operates as a Vietnam stone manufacturer for natural and engineered stone programs. The working model is straightforward: define the material direction, confirm the drawing, control fabrication against the approved requirements, inspect the finished components, then prepare the order for shipment.</p><p>The result is a manufacturing process built around the actual product instead of a generic catalog specification. Vanity tops, kitchen surfaces, furniture pieces, and commercial components all follow the same sequence of material review, drawing approval, production, inspection, and packing.</p><Button variant="secondary" onClick={() => setCurrentTab('factory')}>Review the factory<ArrowRight /></Button></div><figure><picture><source srcSet="/assets/owner/enhanced/vanity-workshop-overhead-enhanced-1280.avif" type="image/avif" /><source srcSet="/assets/owner/enhanced/vanity-workshop-overhead-enhanced-1280.webp" type="image/webp" /><img src="/assets/owner/enhanced/vanity-workshop-overhead-enhanced.jpg" alt="Vanity tops arranged across production worktables" width="1448" height="1086" loading="lazy" /></picture><figcaption>Vanity-top production overview</figcaption></figure></section>}
