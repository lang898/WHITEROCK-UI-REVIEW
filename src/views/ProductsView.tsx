...
export const ProductsView: React.FC<ProductsViewProps> = ({ onSelectProduct, onAddToCart, currentLocale, onToggleCompare, compareIds, setCurrentTab, program }) => {
  ...
  if (!program) {
    return (
      <div className="wr-catalog-page wr-taxonomy-page">
        <header className="wr-landing-hero wr-landing-hero--product">
          <span className="wr-eyebrow">Product programs</span>
          <h1>Stone products built around the drawing.</h1>
          <p>Choose the product family first, then narrow material, finish, dimensions, and use before adding references to an RFQ.</p>
        </header>
        <section className="wr-taxonomy-grid wr-product-program-grid" aria-label="Product categories">
            ...
            return (
              <article className="wr-taxonomy-card" key={definition.name}>
                <a href={routePath(definition.routeId)} onClick={(event) => { event.preventDefault(); setCurrentTab(definition.routeId); }}>
                  <figure><picture>{representative.imageAvif && <source srcSet={representative.imageAvif} type="image/avif" />}{representative.imageWebp && <source srcSet={representative.imageWebp} type="image/webp" />}<img src={representative.image} alt={representative.imageAlt || representative.title} width={representative.imageWidth || 1536} height={representative.imageHeight || 1024} loading="lazy" /></picture></figure>
                  <div className="wr-taxonomy-card__body"><span className="wr-taxonomy-card__meta">{matching.length} references</span><h2>{definition.name}</h2><p>{definition.description}</p><strong>View program<ArrowRight /></strong></div>
                </a>
              </article>
            );
          })}
  ...
  const programProducts = products.filter((product) => productProgramFor(product) === program);
  const materials = useMemo(() => ['All', ...Array.from(new Set(programProducts.map((item) => item.material))).sort()], [program]);
  const finishes = useMemo(() => ['All', ...Array.from(new Set(programProducts.map((item) => textValue(item, 'finish')))).sort()], [program]);
  const sizes = useMemo(() => ['All', ...Array.from(new Set(programProducts.map((item) => textValue(item, 'size')))).sort()], [program]);
  const uses = useMemo(() => ['All', ...Array.from(new Set(programProducts.map((item) => textValue(item, 'use')))).sort()], [program]);
    ...
    <div className="wr-catalog-page">
      ...
      <div className="wr-catalog-layout">
        <aside className={`wr-filter-rail wr-filter-sheet${filtersOpen ? ' is-open' : ''}`} aria-label="Product filters">
          ...
          <button className="wr-button wr-button--ghost" onClick={clearFilters}>Clear filters</button>
