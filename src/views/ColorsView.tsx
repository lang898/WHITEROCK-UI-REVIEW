...
export const ColorsView: React.FC<ColorsViewProps> = ({
  ...
  useEffect(() => { setSelectedFamilies(new Set(family ? [family] : [])); }, [family]);
  ...
  useEffect(() => { const updatePageSize = () => setPageSize(window.innerWidth < 768 ? 4 : 9); window.addEventListener('resize', updatePageSize, { passive: true }); return () => window.removeEventListener('resize', updatePageSize); }, []);
  useEffect(() => { setVisibleCount(pageSize); }, [pageSize, selectedMaterials, selectedFamilies, selectedFinishes, selectedThicknesses, selectedUses, searchQuery]);
  useEffect(() => { if (!previewColor || !filteredColors.some((color) => color.slug === previewColor.slug)) setPreviewColor(filteredColors[0] || null); }, [filteredColors, previewColor]);

  if (!family) {
    const hero = colors.find((color) => color.slug === 'calacatta-crest') || colors[0];
    return (
      <div className="wr-catalog-page wr-color-page wr-taxonomy-page">
        <LandingHero eyebrow={t(currentLocale, 'colorLibrary')} title="Choose a color family." description="Start with the overall palette, then open a family to compare material type, finish, thickness, recommended use, and physical sample options." image={hero.swatchImage} imageWebp={hero.swatchWebp} imageAvif={hero.swatchAvif} imageAlt={`${hero.name} stone color reference`} />
        ...
        <section className="wr-taxonomy-support" aria-labelledby="color-preview-title"><div className="wr-section-heading wr-section-intro"><span className="wr-eyebrow">Visual comparison</span><h2 id="color-preview-title">Preview a selected surface in a room setting.</h2></div><StoneVisualizer currentLocale={currentLocale} onRequestSample={onAddColorSample} /></section>
        <section className="wr-photo-library wr-photo-library--materials" aria-labelledby="physical-material-title"><div className="wr-section-heading wr-section-intro"><span className="wr-eyebrow">Physical material review</span><h2 id="physical-material-title">Slabs and samples from the selection process.</h2><p>Physical references show natural movement, finish response, and lot variation before final sample approval.</p></div><PhotoReferenceRail items={photoGalleries.materialReferences} ariaLabel="Physical slab and stone sample references" aspect="square" /></section>
      </div>
    );
  }

  const currentFamily = familyDefinitions.find((item) => item.name === family)!;
  const familyHero = colors.find((color) => color.slug === currentFamily.representativeSlug) || colors.find((color) => color.colorFamily === family) || colors[0];
  const clearFilters = () => { setSelectedMaterials(new Set()); setSelectedFamilies(new Set(family ? [family] : [])); setSelectedFinishes(new Set()); setSelectedThicknesses(new Set()); setSelectedUses(new Set()); setSearchQuery(''); };
  const activeChips = [
  ...
  const filterGroup = (title: string, values: string[], selected: Set<string>, setter: React.Dispatch<React.SetStateAction<Set<string>>>) => <fieldset><legend>{title}</legend>{values.map((value) => <button type="button" key={value} className={selected.has(value) ? 'is-active' : ''} onClick={() => setter((current) => toggleSetValue(current, value))}><span>{value}</span>{selected.has(value) && <Check />}</button>)}</fieldset>;

  return (
    <div className="wr-catalog-page wr-color-page">
      <LandingHero eyebrow="Color family" title={family} description={currentFamily.description} image={familyHero.swatchImage} imageWebp={familyHero.swatchWebp} imageAvif={familyHero.swatchAvif} imageAlt={`${familyHero.name} ${familyHero.material} color reference`} />
      ...
      <div className="wr-mobile-filter-toolbar"><button className="wr-button wr-button--secondary" type="button" onClick={() => setFiltersOpen(true)}><Filter />Filters{activeFilterCount ? ` (${activeFilterCount})` : ''}</button><span>{filteredColors.length} results · {activeFilterCount} filters</span></div>
      ...
      <div className="wr-catalog-layout">
        <aside className={`wr-filter-rail wr-filter-sheet${filtersOpen ? ' is-open' : ''}`} aria-label="Color filters"><div className="wr-filter-sheet__mobile-header"><strong>Filters</strong><button className="wr-icon-button" onClick={() => setFiltersOpen(false)} aria-label="Close filters"><X /></button></div><div className="wr-filter-rail__heading"><span>{family} colors</span><small>{filteredColors.length} results · {activeFilterCount} filters</small></div><label className="wr-search-input"><Search /><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder={t(currentLocale, 'searchPlaceholder')} /></label>{filterGroup('Material', materials, selectedMaterials, setSelectedMaterials)}{filterGroup('Color family', familyDefinitions.map((item) => item.name), selectedFamilies, setSelectedFamilies)}{filterGroup('Finish', finishes, selectedFinishes, setSelectedFinishes)}{filterGroup('Thickness', thicknesses, selectedThicknesses, setSelectedThicknesses)}{filterGroup('Recommended use', uses, selectedUses, setSelectedUses)}<button className="wr-button wr-button--ghost" type="button" onClick={clearFilters}>Clear filters</button><button className="wr-button wr-button--primary wr-filter-sheet__apply" type="button" onClick={() => setFiltersOpen(false)}>Show {filteredColors.length} results</button>{!compareIds.length && <div className="wr-filter-note wr-compare-empty"><GitCompare /><strong>No comparison selected</strong><p>Select two or three colors to compare specifications and textures side by side.</p></div>}</aside>
        <div className="wr-color-results">
          ...
          <main className="wr-swatch-grid" aria-live="polite">{visibleColors.map((color) => { const compared = compareIds.includes(`color:${color.slug}`); return <article className="wr-swatch-card" key={color.slug} onMouseEnter={() => setPreviewColor(color)} onFocus={() => setPreviewColor(color)}><button className="wr-swatch-card__media" onClick={() => onSelectColor(color)} aria-label={`View ${color.name}`}><ColorSwatchImage color={color} /><span className="wr-catalog-card__sku">{color.material}</span></button><div className="wr-swatch-card__body"><Tag>{color.material}</Tag><small>{color.colorFamily}</small><h2>{color.name}</h2><dl><div><dt>Finish</dt><dd>{color.finishes.slice(0, 2).join(', ')}</dd></div><div><dt>Thickness</dt><dd>{formatMeasurement(color.thicknesses.join(', '))}</dd></div></dl><button className="wr-text-link" onClick={() => setCurrentTab(materialRoute[color.material])}>View {color.material}<ArrowRight /></button><div className="wr-catalog-card__actions"><button className="wr-button wr-button--primary" onClick={() => onAddColorSample(color)}><Package />Order sample</button><button className="wr-button wr-button--secondary" onClick={() => onToggleCompare(color)} aria-pressed={compared}><GitCompare />{compared ? 'Selected' : 'Compare'}</button></div></div></article>; })}{hasMore && <button className="wr-load-more" onClick={() => setVisibleCount((count) => count + pageSize)}><ArrowDown />Show more colors</button>}{!filteredColors.length && <div className="wr-empty-state"><p>No colors match these filters.</p><button className="wr-button wr-button--secondary" onClick={clearFilters}>Clear filters</button></div>}</main>
        </div>
      </div>
    </div>
  );
