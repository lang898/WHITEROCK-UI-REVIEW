...
function routeHtml(template: string, route: RouteDefinition) {
  ...
  if (route.id === 'home') {
    graph.push(
      { '@type': 'Organization', '@id': `${productionDomain}/#organization`, name: legalName, alternateName: displayBrand, url: productionDomain },
      { '@type': 'WebSite', '@id': `${productionDomain}/#website`, name: displayBrand, url: productionDomain, publisher: { '@id': `${productionDomain}/#organization` } },
    );
  } else {
    graph.push({
      '@type': 'BreadcrumbList',
...
const staticRoutePages = () => ({
  ...
  async closeBundle() {
    ...
    const sitemap = [
      ...
      ...routes.map((route) => `  <url><loc>${new URL(route.path, productionDomain).toString()}</loc></url>`),
      '</urlset>',
      '',
    ].join('\n');
